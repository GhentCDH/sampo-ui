...
export const processPortalConfig = async portalConfig => {
  ...
  if (layoutConfig.mainPage) {
    ...
    const {url: bannerImageURL} = await fetch(`/configs/${portalID}/img/${bannerImage}`)
  ...
  if (layoutConfig.topBar.logoImage) {
    ...
    const {url: image} = await fetch(`/configs/${portalID}/img/${layoutConfig.topBar.logoImage}`)
    layoutConfig.topBar.logoImage = image
  }
  if (layoutConfig.topBar.logoImageSecondary) {
    const {url: image} = await fetch(`/configs/${portalID}/img/${layoutConfig.topBar.logoImageSecondary}`)
...
export const createPerspectiveConfig = async ({ portalID, searchPerspectives }) => {
  ...
  for (const perspective of perspectiveConfig) {
    if (has(perspective, 'frontPageImage') && perspective.frontPageImage !== null) {
      const { url: image } = await fetch(`/configs/${portalID}/img/${perspective.frontPageImage}`)
